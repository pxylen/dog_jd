    ...
    for (const v of event["Message"].split("&")) {
      ...
      switch (TENCENTSCF_SOURCE_TYPE) {
        case 'local':
          ...
          break;
        case 'git':
          //2.执行GITHUB111远端的js文件(因GITHUB111的raw类型的文件被墙,此方法云函数不推荐)
          request(`https://raw.GITHUB111usercontent.com/LXK9301/jd_scripts/master/${v}.js`, function (error, response, body) {
            eval(response.body)
          })
          break;
        case 'custom':
