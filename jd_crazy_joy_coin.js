...
crazy joy
鎸傛満棰嗛噾甯�/瀹濈涓撶敤
娲诲姩鍏ュ彛锛氫含涓淎PP鎴戠殑-鏇村宸ュ叿-鐤媯鐨凧OY
鈿狅笍寤鸿浜戠浣跨敤銆傛墜鏈虹涓嶅缓璁娇鐢�(浼氫竴鐩磋窇涓嬪幓锛屾案涓嶅仠姝�)
10 7 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_crazy_joy_coin.js

 */


const $ = new Env('crazyJoy鎸傛満');
  ...
  while (true) {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
      if (cookiesArr[i]) {
        ...
        if (!$.isLogin) {
         $.log(`\n浜笢璐﹀彿${$.index} ${$.nickName || $.UserName}\ncookie宸茶繃鏈�,璇烽噸鏂扮櫥褰曡幏鍙朶n`)
          continue
        }
        await jdCrazyJoy()
      }
...
async function jdCrazyJoy() {
  ...
  // 濡傛灉鏍煎瓙鍏ㄩ儴琚崰鏈変笖娌℃湁鍙互鍚堝苟鐨凧OY锛屽彧鑳藉洖鏀朵綆绾х殑JOY (涓旀渶浣庣瓑绾х殑JOY灏忎簬30绾�)
  if(checkHasFullOccupied() && !checkCanMerge() && finMinJoyLevel() < 30) {
    const minJoyId = Math.min(...$.joyIds);
    const boxId = $.joyIds.indexOf(minJoyId);
    console.log(`鏍煎瓙鍏ㄩ儴琚崰鏈変笖娌℃湁鍙互鍚堝苟鐨凧OY锛屽洖鏀�${boxId + 1}鍙蜂綅绛夌骇涓�${minJoyId}鐨凧OY`)
  ...
  for (let idx in obj) {
    ...
    if (idx === '34' && vo.length >= 8) {
      if ($.coin >= 6000000000000000) {
        //褰撳瓨鍦�8涓�34绾OY锛屽苟涓斿墿浣欓噾甯佸彲涓哄悗闈㈢户缁悎鎴愪袱鍙柊鐨�34绾OY(鎸夊叏閮ㄧ敤30绾OY鍚堟垚涓�鍙�34绾OY璁＄畻闇�:1.66T * 2 * 2 * 2 * 2 = 26.56T = 2.6Q)鏃�,鍒欐鏉′欢涓嬪悎骞朵袱涓�34绾OY
        $.log(`寮�濮嬪悎骞朵袱鍙�${idx}绾oy\n`)
        await mergeJoy(vo[0], vo[1])
        await $.wait(3000)
        await getJoyList()
  ...
  await getUserBean()
  await $.wait(5000)
  console.log(`褰撳墠淇℃伅锛�${$.bean} 浜眴锛�${$.coin} 閲戝竵`)
}
//鏌ヨ鏍煎瓙閲岄潰鏄惁杩樻湁绌烘牸
...
//鏌ユ壘褰撳墠 璐拱 joyLists 涓渶浣庣瓑绾х殑閭ｄ竴涓�
function finMinJoyLevel() {
  return Math.min(...$.joyIds.filter(s => s))
}
/**
 * 鏉ユ簮锛歨ttps://elecv2.ml/#绠楁硶鐮旂┒涔嬪悎骞剁被灏忔父鎴忕殑鏈�浼樿喘涔伴棶棰�
 * 鑾峰彇涓嬩竴涓悎閫傜殑璐拱绛夌骇銆傦紙绠楁硶浜屼紭鍖栫増锛�
 * @param     {array}     joyPrices    鍟嗗簵 joy 浠锋牸鍜岀瓑绾у垪琛�
 * @param     {number}    start        寮�濮嬫瘮杈冪殑绛夌骇銆傝寖鍥�1~30锛岄粯璁わ細30
 * @param     {number}    direction    鍚戜笂姣旇緝杩樻槸鍚戜笅姣旇緝銆�0锛氬悜涓嬫瘮杈冿紝1锛氬悜涓婃瘮杈冿紝榛樿锛�0
 * @return    {number}                 杩斿洖鏈�缁堥�傚悎璐拱鐨勭瓑绾�
 */
function getBuyid2b(joyPrices, start = 30, direction = 0) {
  if (start < 1 || start > 30) {
    console.log('start 绛夌骇杈撳叆涓嶅悎娉�')
    return 1
  }
  let maxL = 30        // 璁剧疆鏈�楂樿喘涔扮瓑绾�
  if (direction) {
    // 鍚戜笂姣旇緝
    for (let ind = start - 1; ind < maxL - 1; ind++) {       // 鍟嗗簵 joy 绛夌骇鍜屽簭鍒楀彿鐩稿樊1锛岄渶瑕佸噺涓�涓�
    ...
    if (zeroNum === 0) {
      ...
      while (zeroNum--) {
        ...
        if (zeroNum >= 2 && joyPrices && joyPrices.length) {
          // buyLevel = getBuyid2b(joyPrices, joyPrices.length)     // 鍏蜂綋鍙傛暟鍙牴鎹釜浜烘儏鍐佃繘琛岃皟鏁�
          buyLevel = getBuyid2b(joyPrices)     // 鍏蜂綋鍙傛暟鍙牴鎹釜浜烘儏鍐佃繘琛岃皟鏁�
        }
        if ($.joyPrices) {
          //娣诲姞鍒ゆ柇銆傞伩鍏嶅湪鑾峰彇$.joyPrices澶辫触鏃讹紝鐩存帴涔扮瓑绾�1
      ...
      try {
        if (err) {
          ...
          if (data.success && data.data && data.data.shop) {
            ...
            $.joyPrices = shop;
            $.buyJoyLevel = shop.length ? shop[shop.length - 1]['joyId'] : 1;//鍙喘涔扮殑鏈�澶х瓑绾�
            if ($.isNode() && process.env.BUY_JOY_LEVEL) {
              $.log(`褰撳墠鍙喘涔扮殑鏈�楂楯OY绛夌骇涓�${$.buyJoyLevel}绾n`)
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data.newJoyId) {
              if (data.data.newJoyId > 34) {
                ...
                console.log(`鍚堝苟鎴愬姛锛岃幏寰�${level(data.data.newJoyId)}绾oy`)
                if (data.data.newJoyId === 1007 && $.isNode()) await notify.sendNotify($.name, `浜笢璐﹀彿${$.index} ${$.nickName}\n鍚堝苟鎴愬姛锛岃幏寰�${level(data.data.newJoyId)}绾oy`)
              } else {
                console.log(`鍚堝苟鎴愬姛锛岃幏寰�${data.data.newJoyId}绾oy`)
              }
            } else
              console.log(`鍚堝苟澶辫触锛岄敊璇痐)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
      ...
      try {
        if (err) {
          ...
          if (data.success) {
            ...
            $.log(`璐拱${joyId}绾oy鎴愬姛锛屽墿浣欓噾甯併��${data.data.totalCoins}銆慲)
      ...
      try {
        if (err) {
          ...
          if (data.success) {
            ...
            $.log(`鍥炴敹${joyId}绾oy鎴愬姛锛屽墿浣欓噾甯併��${data.data.totalCoins}銆慲)
            $.coin = data.data.totalCoins
          } else {
            console.log(data.message)
          }
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.success)
              console.log(`閲戝竵琛ョ粰棰嗗彇鎴愬姛锛岃幏寰�${data.data.coins}閲戝竵`)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.data && data.data.tryMoneyJoyBeans) {
              console.log(`鍒嗙孩鐙楃敓鏁堜腑锛岄璁¤幏寰� ${data.data.tryMoneyJoyBeans} 浜眴濂栧姳`)
            }
            if (data.data && data.data.totalCoinAmount) {
              $.coin = data.data.totalCoinAmount;
              $.log(`褰撳墠閲戝竵:${$.coin}\n`)
            } else {
              $.coin = `鑾峰彇褰撳墠閲戝竵鏁伴噺澶辫触`
            }
            ...
            if (data.data) {
              $.log(`姝ゆ鍦ㄧ嚎鏀剁泭锛氳幏寰� ${data.data['coins']} 閲戝竵`)
            }
          }
        }
      } catch (e) {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            // data = JSON.parse(data);
            // if (data.data && data.data.tryMoneyJoyBeans) {
            //   console.log(`銆愪含涓滆处鍙� ${userName}銆戝垎绾㈢嫍鐢熸晥涓紝棰勮鑾峰緱 ${data.data.tryMoneyJoyBeans} 浜眴濂栧姳`)
            // }
            // if (data.data) {
            //   $.log(`銆愪含涓滆处鍙� ${userName}銆戞娆″湪绾挎敹鐩婏細鑾峰緱 ${data.data['coins']} 閲戝竵`)
            // }
          }
        }
      } catch (e) {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data['success']) {
              $.log(`鐐瑰嚮骞歌繍鐩掑瓙鎴愬姛锛屽墿浣欒鐪嬭棰戞鏁帮細${data.data.advertViewTimes}, ${data.data.advertViewTimes > 0 ? '绛夊緟32绉�' : '璺冲嚭'}`)
    ...
    $.get(taskUrl('crazyJoy_event_obtainAward', JSON.stringify(body)), async (err, resp, data) => {
      try {
        if (err) {
          ...
          $.log(`${$.name} API璇锋眰澶辫触锛岃妫�鏌ョ綉璺噸璇昤)
        } else {
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data['success']) {
              $.log(`骞歌繍鐩掑瓙濂栧姳棰嗗彇鎴愬姛锛岃幏寰楋細${data.data.beans}浜眴锛�${data.data.coins}閲戝竵`)
            } else {
              $.log(`骞歌繍鐩掑瓙濂栧姳棰嗗彇澶辫触锛岄敊璇俊鎭細${data.message || JSON.stringify(data)}`)
            }
          }
        }
      } catch (e) {
          ...
          if (safeGet(data)) {
            ...
            if (data['success'] && data.data) {
              for(let vo of data.data){
                if(vo['status']){
                  console.log(`${vo['joyId']}鍗囩骇濂栧姳鍙互棰嗗彇`)
                }
              }
            } else {
              $.log(`骞歌繍鐩掑瓙濂栧姳棰嗗彇澶辫触锛岄敊璇俊鎭細${data.message || JSON.stringify(data)}`)
