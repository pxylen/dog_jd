/**
 * 鎵爜鑾峰彇浜笢cookie锛屾鏂瑰紡寰楀埌鐨刢ookie鏈夋晥鏈熶负90澶�(瀹為檯寰呮祴璇�)
    ...
    $.post(taskPostUrl(), (err, resp, data) => {
      try {
        if (err) {
          ...
          qrcode.generate(url, {small: true}); // 杈撳嚭浜岀淮鐮�
          console.log("璇锋墦寮� 浜笢APP 鎵爜鐧诲綍(浜岀淮鐮佹湁鏁堟湡涓�3鍒嗛挓)");
          console.log(`\n\n娉細濡傛壂鎻忎笉鍒帮紝璇蜂娇鐢ㄥ伐鍏�(渚嬪鍦ㄧ嚎浜岀淮鐮佸伐鍏凤細https://cli.im)鎵嬪姩鐢熸垚濡備笅url浜岀淮鐮乗n\n${url}\n\n`);
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
...
function getCookie() {
  $.timer = setInterval(async () => {
    ...
    if (checkRes['errcode'] === 0) {
      ...
      $.log(`浜岀淮鐮佸凡澶辨晥锛岃閲嶆柊鑾峰彇浜岀淮鐮侀噸鏂版壂鎻廫n`);
      clearInterval($.timer);
      $.done();
    } else if (checkRes['errcode'] === 176) {
      //鏈壂鎻忕櫥褰�
    } else {
      $.log(`鍏朵粬寮傚父锛�${JSON.stringify(checkRes)}\n`);
      clearInterval($.timer);
      $.done();
    }
  }, 1000)
...
function formatCookie(headers) {
  new Promise(resolve => {
    ...
    $.log(`浜笢鐢ㄦ埛锛�${$.UserName} Cookie鑾峰彇鎴愬姛(鏈夋晥鏈燂細${headers['strict-transport-security'].substring("max-age=7776000".indexOf('=') + 1, "max-age=7776000".length)}绉�)锛宑ookie濡備笅锛歚);
    $.log(`\n${cookie1}\n`);
    resolve()
  })
}
